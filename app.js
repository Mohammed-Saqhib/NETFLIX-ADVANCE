...
document.addEventListener('DOMContentLoaded', function() {
    // Create ultra-authentic Netflix intro splash screen
    const splashScreen = document.querySelector('.netflix-splash');
    const splashContainer = document.querySelector('.netflix-splash-container');
    
    // Create Netflix N logo container
    const nLogoContainer = document.createElement('div');
    nLogoContainer.className = 'netflix-n-logo-container';
    
    // Create Netflix N logo SVG with exact Netflix proportions
    const netflixLogo = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    netflixLogo.setAttribute("class", "netflix-n-logo");
    netflixLogo.setAttribute("viewBox", "0 0 24 35");
    netflixLogo.setAttribute("fill", "none");
    netflixLogo.setAttribute("xmlns", "http://www.w3.org/2000/svg");
    
    // Path for the exact Netflix N logo
    const logoPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
    logoPath.setAttribute("d", "M3 0V35L8 32V0H3ZM16 0V35L21 32V0H16Z");
    logoPath.setAttribute("fill", "#E50914");
    
    const logoDiagonal = document.createElementNS("http://www.w3.org/2000/svg", "path");
    logoDiagonal.setAttribute("d", "M3 7L8 4L21 29L16 32L3 7Z");
    logoDiagonal.setAttribute("fill", "#E50914");
    
    netflixLogo.appendChild(logoPath);
    netflixLogo.appendChild(logoDiagonal);
    nLogoContainer.appendChild(netflixLogo);
    
    // Create shine effect element
    const logoShine = document.createElement('div');
    logoShine.className = 'netflix-logo-shine';
    nLogoContainer.appendChild(logoShine);
    
    // Add Netflix logo container to splash
    splashContainer.appendChild(nLogoContainer);
    
    // Create text container for "Netflix Household Verify"
    const textContainer = document.createElement('div');
    textContainer.className = 'netflix-text-container';
    
    const logoText = document.createElement('span');
    logoText.className = 'netflix-logo-text';
    logoText.textContent = 'Netflix Household Verify';
    textContainer.appendChild(logoText);
    splashContainer.appendChild(textContainer);
    
    // Add the boom effect element
    const boomEffect = document.createElement('div');
    boomEffect.className = 'netflix-boom';
    splashScreen.appendChild(boomEffect);
    ...
    // After authentic Netflix animation completes, fade out splash screen
    setTimeout(() => {
        splashScreen.classList.add('fade-out');
        
        // Remove splash and show content after fade completes
        setTimeout(() => {
            document.body.style.overflow = '';
            splashScreen.style.display = 'none';
            
            // Fade in main content with proper Netflix timing
            mainContent.forEach(el => {
                el.classList.add('content-fade-in');
            });
            
        }, 1000); // Match with CSS fade-out duration
    }, 4500); // Match with complete animation sequence
...
function initStepToggle() {
    ...
    stepButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            ...
            // Log to ensure we have the right elements
            console.log(`Button clicked for ${method}, steps container:`, stepsContainer);
            ...
            if (isVisible) {
                ...
                console.log(`${method} steps hidden`);
            } else {
                ...
                console.log(`${method} steps shown`);
                ...
                // Add animation effect for showing
    ...
    // Ensure the steps containers are hidden initially but ready to be shown
    document.querySelectorAll('.steps-container').forEach(container => {
        container.style.display = 'none';
        // Don't set opacity to 0 here as it can cause issues with showing later
    });
}
...
function initFAQAccordion() {
    ...
    // First, ensure all FAQ answers are hidden initially
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Force initialize view-steps buttons
    document.querySelectorAll('.view-steps-btn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            const method = this.getAttribute('data-method');
            const stepsContainer = document.getElementById(`${method}-steps`);
            console.log(`Extra click handler for ${method} button`);
